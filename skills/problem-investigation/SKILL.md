...
6. For a bug, performance regression, or security defect, reproduce it or create equivalent bounded proof. Never experiment on production. Browser tests use synthetic data only.
...
10. Before implementation, establish the regression baseline: preferably the same test fails on the exact bad base. Where technically impractical, record an explicit controller-reviewable exception with equivalent evidence. A same-SHA FAIL→PASS rerun is flaky, not RED→GREEN proof.
...
- Missing business intent, credentials, protected runtime access, or irreversible authorization: isolate only the affected task in `WAITING_EXTERNAL_DEPENDENCY`; continue unrelated safe work.
