...
def test_failed_machine_check_blocks_all_review_nodes(tmp_path):
    ...
    continuity = ContinuityStore(tmp_path / "continuity.db")
    task = continuity.submit_task(repository=str(repo), title="bad whitespace", contract=contract, risk="R2", task_id="task", idempotency_key="task")
    plan = build_mesh_plan(
        task_id=task["task_id"], risk="R2", contract=contract,
        prompt_root=".delivery/ztad/tasks/task/prompts", output_schema=str(SCHEMA),
        check_config=".delivery/ztad/config.json", command_policy=str(ROOT / "policies/command-policy.yaml"),
        maximum_parallel_writers=1, maximum_plan_candidates=1,
    )
    write_mesh_plan(plan, repository=repo, output_file=repo / ".delivery/ztad/tasks/task/plan.json")
    # Force all nodes to use the committed base candidate.
    specs = []
    for node in plan.nodes:
        metadata = dict(node.metadata)
        metadata.update({"base_sha": base, "head_sha": base, "context_id": "sha256:" + "c" * 64})
        specs.append(MeshNodeSpec.create(
            node_id=node.node_id, task_id=node.task_id, title=node.title, task_family=node.task_family,
            role=node.role, risk=node.risk, write_access=node.write_access, scopes=node.scopes,
            prompt_path=node.prompt_path, output_schema=node.output_schema, priority=node.priority,
            metadata=metadata, dependencies=node.dependencies, idempotency_key=node.idempotency_key,
        ))
    mesh = MeshStore(tmp_path / "mesh.db")
    mesh.submit_graph(specs)
    ...
    runtime = MeshRuntime(
        ...
        worker_id="worker", output_root=tmp_path / "outputs", global_parallel_cap=8,
    )
    report = runtime.run_until_idle(maximum_ticks=30, maximum_seconds=30)
    check_nodes = [node for node in mesh.list_nodes(task_id="task") if node["role"] == "check_runner"]
    review_nodes = [node for node in mesh.list_nodes(task_id="task") if node["role"] == "supervisor"]
    assert check_nodes[0]["state"] in {"RETRY_READY", "RETRY_WAIT", "QUARANTINED"}
    assert all(node["state"] == "READY" for node in review_nodes)
    assert provider.review_calls == 0
    assert report["mesh_status"]["blocked_by_dependencies"] >= len(review_nodes)
